...
public static class Recursion
{
    ...
    public static int SumSquaresRecursive(int n)
    {
        ...
        // Recursive case: add n^2 to sum of squares of smaller numbers
    ...
    public static void PermutationsChoose(List<string> results, string letters, int size, string word = "")
    {
        // TODO Start Problem 2
         if (word.Length == size)
        {
            results.Add(word); // Base case: reached desired length
            return;
        }
        ...
        for (int i = 0; i < letters.Length; i++)
        {
            ...
            // Recursive call: add letter and reduce remaining letters
        }
    }
    ...
    public static decimal CountWaysToClimb(int s, Dictionary<int, decimal>? remember = null)
    {
        ...
        // Store the result in the dictionary to avoid recalculating
        remember[s] = ways;
    ...
    public static void WildcardBinary(string pattern, List<string> results)
    {
        ...
        if (index == -1)
        {
            results.Add(pattern); // Base case: no wildcards left
            return;
        }
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        currPath.Add((x, y)); // Add current position

        if (maze.IsEnd(x, y))
        {
            results.Add(currPath.AsString()); 
            // Base case: reached end, add path using assignment-required AsString()
        }
        else
        {
        // Recursive case: explore all four directions
